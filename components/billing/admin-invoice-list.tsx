...
export function AdminInvoiceList({ invoices }: { invoices: InvoiceRecord[] }) {
  if (invoices.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-[2rem] border border-border/80 bg-card/70 py-16 text-center">
        ...
        <div className="mt-3 text-sm font-medium">No invoices found</div>
        <p className="mt-1 max-w-xs text-xs text-muted-foreground">
          Invoices are created when an order is confirmed and sent to the client.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="grid gap-4">
      {invoices.map((invoice) => {
        const invoiceHref = `/admin/invoices/${invoice.id}` as Route;
