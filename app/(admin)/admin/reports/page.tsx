...
async function getReportData() {
  ...
  return {
    ...
    monthBilled: Number(monthRevenue._sum.totalAmount ?? 0),
    lastMonthRevenue: Number(lastMonthRevenue._sum.paidAmount ?? 0),
    allTimePaid: Number(allTimeRevenue._sum.paidAmount ?? 0),
    allTimeBilled: Number(allTimeRevenue._sum.totalAmount ?? 0),
    designerPerformance,
    recentActivity,
    topClients: topClients.map((c) => ({
      name: clientNameMap[c.clientUserId] ?? "Unknown",
      count: c._count.id,
    })),
  };
}
        ...
        {[
          {
            ...
            sub: monthGrowth !== null ? `${monthGrowth >= 0 ? "+" : ""}${monthGrowth}% vs last month` : "First month",
          },
          {
            label: "This month (billed)",
            value: `$${data.monthBilled.toLocaleString()}`,
            sub: "invoiced this month",
          },
          {
            label: "All-time collected",
            value: `$${data.allTimePaid.toLocaleString()}`,
            sub: `$${data.allTimeBilled.toLocaleString()} billed total`,
          },
        ].map((s) => (
          <div key={s.label} className="rounded-[1.75rem] border border-border/80 bg-card/70 p-5">
          ...
          <div className="overflow-hidden rounded-[2rem] border border-border/80 bg-card/70">
            <div className="divide-y divide-border/80">
              ...
              {data.ordersByStatus.length === 0 && (
                <div className="px-5 py-6 text-center text-sm text-muted-foreground">No orders yet.</div>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="overflow-hidden rounded-[2rem] border border-border/80 bg-card/70">
            <div className="divide-y divide-border/80">
              ...
              {data.topClients.length === 0 && (
                <div className="px-5 py-6 text-center text-sm text-muted-foreground">No data yet.</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-[2rem] border border-border/80 bg-card/70">
          ...
          <div className="divide-y divide-border/80">
            {data.designerPerformance.map((d) => {
              ...
              const active = d.assignedOrders.filter((o) =>
                ["SUBMITTED", "IN_PROGRESS", "PROOF_READY", "REVISION_REQUESTED"].includes(o.status)
              ).length;
              const delivered = d.assignedOrders.filter((o) =>
              ...
              return (
                <div
                  key={d.id}
                  className="grid grid-cols-1 gap-1 px-5 py-4 text-sm sm:grid-cols-[2fr_1fr_1fr_1fr_1fr] sm:items-center"
                >
                  <div className="font-medium">{d.name ?? d.email ?? d.id}</div>
                  <div className="text-muted-foreground">{total}</div>
                  <div className="text-amber-400">{active}</div>
                  <div className="text-emerald-400">{delivered}</div>
                  <div>
                    <span
                      className={`text-sm font-semibold ${rate >= 80 ? "text-emerald-400" : rate >= 50 ? "text-amber-400" : "text-muted-foreground"}`}
                    >
                      {total > 0 ? `${rate}%` : "—"}
                    </span>
                  </div>
            ...
            {data.designerPerformance.length === 0 && (
              <div className="px-5 py-8 text-center text-sm text-muted-foreground">No designers yet.</div>
            )}
          </div>
        </div>
      </section>
            ...
            {data.recentActivity.map((log) => (
              <div key={log.id} className="flex items-center gap-4 px-5 py-3 text-sm">
                ...
                <div className="flex-1">
                  ...
                  {log.actorUser?.name && (
                    <span className="ml-2 text-xs text-muted-foreground">by {log.actorUser.name}</span>
                  )}
                </div>
