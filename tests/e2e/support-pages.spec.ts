...
test("client support page renders", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/client\/support$/, { timeout: 15000 });

  await expect(
    page.getByRole("heading", {
      name: /support conversations now live inside the portal/i,
    })
  ).toBeVisible();
});

test("admin support page renders", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/admin\/support$/, { timeout: 15000 });
