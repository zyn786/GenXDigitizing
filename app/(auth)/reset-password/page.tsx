...
export const metadata: Metadata = {
  title: buildTitle("Reset Password"),
};
...
function getErrorMessage(error?: string) {
  switch (error) {
    case "invalid-reset-form":
      return "Please complete the reset form correctly.";
...
export default async function ResetPasswordPage({
  ...
  const hasToken = Boolean(params.email && params.token);
        ...
        <section className="w-full max-w-[560px] rounded-2xl border border-border/60 bg-card p-6 shadow-lg md:p-8">
          <div className="text-center">
            ...
            <p className="mt-3 text-sm leading-6 text-white/60">
              Choose a new password for your GenX Digitizing account.
            </p>
          </div>
