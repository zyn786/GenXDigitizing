    ...
    <section className="relative isolate overflow-hidden bg-[#f7f7fb] px-4 py-10 text-slate-950 dark:bg-[#050814] dark:text-white sm:py-12 md:px-8 md:py-24 lg:py-28">
      ...
      <div className="page-shell relative z-10">
        ...
        >
          <Link
            href="/portfolio"
            className="inline-flex min-h-[40px] items-center justify-center rounded-full bg-slate-950 px-4 py-2.5 text-xs font-bold text-white shadow-lg shadow-slate-950/15 transition hover:-translate-y-0.5 hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-white/90 md:min-h-[46px] md:px-5 md:py-3 md:text-sm"
          >
          ...
          <Link
            href="/order"
            className="inline-flex min-h-[40px] items-center justify-center rounded-full border border-slate-300 bg-white/70 px-4 py-2.5 text-xs font-bold text-slate-800 shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:bg-white dark:border-white/10 dark:bg-white/[0.06] dark:text-white dark:hover:bg-white/[0.1] md:min-h-[46px] md:px-5 md:py-3 md:text-sm"
          >
            Start an order
          </Link>
        </motion.div>

        <p className="mx-auto mt-4 max-w-2xl text-center text-[11px] leading-5 text-slate-500 dark:text-white/35 md:mt-5 md:text-xs md:leading-6">
...
function TestimonialCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={[
          "relative h-full overflow-hidden border border-slate-200 bg-white/80 shadow-sm shadow-slate-950/5 backdrop-blur-xl transition-all duration-300 dark:border-white/[0.08] dark:bg-white/[0.045] dark:shadow-black/20",
          mobile
            ? "rounded-[1.5rem] p-4"
            : "rounded-[2rem] p-6 md:p-7 md:group-hover:-translate-y-1 md:group-hover:border-slate-300 md:group-hover:shadow-2xl md:group-hover:shadow-slate-950/10 dark:md:group-hover:border-white/[0.16] dark:md:group-hover:shadow-black/40",
        ].join(" ")}
      >
        <div className="pointer-events-none absolute right-0 top-0 h-24 w-24 rounded-bl-full bg-gradient-to-br from-slate-100 to-transparent opacity-70 dark:from-white/[0.06] md:h-28 md:w-28 md:opacity-80" />
        ...
        <div className="relative z-10">
          <div className="flex items-start justify-between gap-4">
            <div className="flex gap-0.5">
              {Array.from({ length: 5 }).map((_, starIndex) => (
                <Star
                  key={starIndex}
                  className="h-3.5 w-3.5 fill-amber-400 text-amber-400 md:h-4 md:w-4"
                />
              ))}
            </div>

            <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-slate-400 dark:border-white/10 dark:bg-white/[0.04] dark:text-white/30 md:h-10 md:w-10 md:rounded-2xl">
              <Quote className="h-4 w-4" />
            </div>
          </div>

          <blockquote className="mt-4 text-[13px] leading-6 text-slate-600 dark:text-white/58 md:mt-6 md:text-sm md:leading-7">
            “{item.quote}”
          </blockquote>

          <div className="mt-5 h-px w-full bg-gradient-to-r from-slate-200 via-slate-300 to-transparent dark:from-white/10 dark:via-white/15 md:mt-6" />
          ...
          <div className="mt-4 inline-flex rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[9px] font-black uppercase tracking-[0.16em] text-slate-400 dark:border-white/10 dark:bg-white/[0.04] dark:text-white/30 md:mt-5 md:text-[10px] md:tracking-[0.18em]">
            Preview feedback
          </div>
        </div>
      </div>
