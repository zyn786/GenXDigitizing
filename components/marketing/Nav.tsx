...
import { ChevronRight, Star, Shield, Bell, LayoutDashboard } from "lucide-react";
        ...
        {open && (
          ...
          >
            <div className="px-5 sm:px-6 py-4 flex flex-col gap-1">
                    ...
                    {isExpandable ? (
                      <>
                        <button
                          ...
                          className={cn(
                            "w-full flex items-center gap-3 py-3.5 px-3 rounded-xl text-sm font-semibold transition-all duration-150 bg-transparent border-none cursor-pointer",
                            isActive
                        ...
                        >
                          {l.label} ▼
                          {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-gradient-to-r from-[#2563EB] to-[#7C3AED]" />}
                        </button>
                        {isExpanded && (
                          <div className="ml-4 border-l border-[var(--border)] pl-3 mb-1">
                            {subItems.map((sub) => (
                              <Link
                                ...
                                className="flex items-center gap-2 py-2.5 px-3 rounded-lg text-[13px] font-medium text-[var(--txt2)] hover:text-[var(--txt)] hover:bg-[var(--elevated)]/50 no-underline transition-colors"
                              >
                                <ChevronRight size={12} className="text-[var(--txt3)]" />
                                {sub.label}
                              </Link>
                            ))}
                          </div>
              ...
              {/* Trust stats */}
              <div className="flex items-center justify-center gap-3 py-3 mt-2 border-t border-[var(--border)] text-[11px] text-[var(--txt2)]">
                <span>⭐ {SITE_STATS.avgRating}/5</span>
                <span className="text-[var(--border3)]">|</span>
                <span>🧵 {fmtPlus(SITE_STATS.ordersCompleted)}+ Orders</span>
                <span className="text-[var(--border3)]">|</span>
                <span>🌍 {SITE_STATS.countriesServed}+ Countries</span>
              </div>
